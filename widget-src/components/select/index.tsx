        ...
        {chevron}
        {
            open
                ...
                <AutoLayout {...list} name="Select Option List">
                    <AutoLayout {...item} key={""} onClick={() => { setSelected(null) }}>
                        <Text {...text} fill={colors.neutral[400]}>{placeholder}</Text>
                    </AutoLayout>
                    {options.map((option, index) => (
                        <AutoLayout {...item} name="Select Item" key={index} onClick={() => { select(option) }}>
