    ...
    return (
        <AutoLayout {...docStyle} name={"Switch Doc"}>
            ...
            <AutoLayout direction="vertical" spacing={4} width={"fill-parent"}>
                ...
                <P>· Pass a <Pre>defaultChecked</Pre> prop to set the initial state of a Switch.</P>
                <P>· Pass a <Pre>onChange</Pre> callback to get</P>
            </AutoLayout>
            <AutoLayout width={"fill-parent"} fill={colors.blue[50]} padding={16} cornerRadius={6}>
                <P fill={colors.blue[900]}>
                    Notice: A unique <Span {...codeStyle}>id</Span> is required to differentiate states of multiple Switch components.
                </P>
            </AutoLayout>
            <Switch id="ht5o" onChange={(checked) => {
                figma.notify(`Switched ${checked ? "on" : "off"}.`)
            }} />

            <Heading as="h5">Controlled Switch</Heading>
            <P>Pass a <Pre>checked</Pre> prop to control the state of a Switch. Controlled Switch components do not require an <Pre>id</Pre>.</P>
            ...
            <P>Switch component comes in three sizes: <Pre>sm(16px), md(20px)</Pre> and <Pre>lg(28px)</Pre>.</P>
            ...
            <P onClick={() => setCurrentPage("colors")}>Pass a <Pre>colorScheme</Pre> prop to set the background color of Switch components. See <Span fill={colors.blue[500]}>Colors</Span> for available color schemes.</P>
            <AutoLayout spacing={8}>
