...
const isAvatar = (component: any) => {
    if ("props" in component) {
        if ("name" in component.props && (component.props.name.startsWith("Avatar"))) { return true }
    }
    return false
}
...
export function Avatar({
    ...
    const { avatar: { padding, ...rest } } = getAvatarStyles({ size })
        ...
        >
            {
                initials
                    ...
                    : typeof fill === "object" ? null : <IconUser width={"fill-parent"} height={"fill-parent"} color={colors.white} />
            }
        </AutoLayout>
    )
...
export function AvatarGroup(
    { children, max = 5, size = "md", stacked = false, spacing, ...rest }: AvatarGroupProps
) {
