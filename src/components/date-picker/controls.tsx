...
export function Controls(
    ...
    return <AutoLayout name="Calendar Controls" {...controls}>
        {view !== "month" && <IconButton variant="ghost" size="sm" icon={<IconChevronLeft />} onClick={() => { view === "year" ? paginate(-108) : paginate(-1) }} />}
        {
            view === "year"
                ? <Text
                    {...buttonStyles}
                ...
                : view === "date"
                    ? (
                        <AutoLayout width={"fill-parent"} horizontalAlignItems={"center"} spacing={4}>
                            <Text {...buttonStyles} onClick={() => { setView('month') }}>{CALENDAR_MONTHS[dayjs(pivot).month()]}</Text>
                            <Text {...buttonStyles} onClick={() => { setView("year") }}>{dayjs(pivot).year() + ""}</Text>
                        </AutoLayout>
                    )
        ...
        {
            view !== "month"
            && <IconButton variant="ghost" size="sm" icon={<IconChevronRight />} onClick={() => { view === "year" ? paginate(108) : paginate(1) }} />}
    </AutoLayout>
}