...
export function DatePicker(props: DatePickerProps) {
    const { variant, size, disabled, onChange } = props;

    /*---- Styles ----*/
    const { field, input } = getFieldStyles({ variant, size, disabled });
    const { cell, calendar, digit, controls, weekday, indicator } = getDatePickerStyles();
                            ...
                            <AutoLayout key={index} width={"fill-parent"} name="Calendar Row" spacing={2}>
                                {
                                    ...
                                    week.map((day, index) => {
                                        ...
                                        >
                                            {isToday
                                                ? <Ellipse name="Today Indicator" {...indicator} />
                                                : null}
                                            <Text {...digit} fill={selected ? colors.white : isInThisMonth ? colors.neutral[900] : colors.neutral[300]}>
                                                {day.getDate()}
                                            </Text>
                                        </AutoLayout>
                                    })
