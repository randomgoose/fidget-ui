...
const { widget } = figma
const { AutoLayout, useSyncedState, Text, Span } = widget
    ...
    return <AutoLayout {...docStyle} name={"Select Doc"}>
        ...
        <AutoLayout width={"fill-parent"} fill={colors.blue[50]} padding={16} cornerRadius={6}>
            <P fill={colors.blue[900]}>
                Notice: An <Span {...codeStyle}>id</Span> prop is required for each Select component as the component keeps an internal state to conditionally render the dropdown list.
            </P>
        </AutoLayout>

        <Heading as="h5">Usage</Heading>

        <P>Pass in an array of options and get user selection from the <Pre>onChange</Pre> callback.</P>
        <Select
            id={"doc/select/a"}
        ...
        <AutoLayout spacing={24} overflow={"visible"}>
            {
                ["filled", "outline", "flush"].map(variant => (
                    <Select
                        variant={variant as FieldProps['variant']}
                        key={variant}
                        placeholder="Select a letter"
                        width={128}
        ...
        <P>You may notice some of the poppers are blocked by elements below. This is because the default canvas stacking order in Figma widets is "last on top".</P>
        <P>In this case, you can set the <Pre>placement</Pre> prop as <Pre>top</Pre> to place the popper on the other side.</P>
        <Select
            id={"doc/select/top"}
            placeholder="Select a letter"
            width={240}
