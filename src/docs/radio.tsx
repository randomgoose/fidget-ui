...
export function RadioDoc() {
    ...
    >
        ...
        <P>Provide a series of radios with <Pre>RadioGroup</Pre> component.  A unique <Pre>name</Pre> is required as the component keeps the selection in an internal state, which uses name as a key.</P>
        <P>Pass in an array of options to <Pre>options</Pre> prop to render them as radios.</P>
        <RadioGroup
            name={"radio-doc/a"}
            options={[
...
return <RadioGroup
    ...
    onChange={({ value }) => figma.notify(value)}/>`
        ...
        <AutoLayout spacing={8}>
            <Button variant="outline" onClick={() => setAnimals("dogs")}>Select dogs</Button>
            <Button variant="outline" onClick={() => setAnimals("cats")}>Select cats</Button>
            <Button variant="outline" onClick={() => setAnimals("sheep")}>Select sheep</Button>
        </AutoLayout>
        <RadioGroup
            name={"radio-doc/b"}
            value={animals}
        ...
        <P>By default, the radios are horizontally placed. Change the <Pre>orientation</Pre> prop to <Pre>vertical</Pre> to vertically stack radios.</P>
        <RadioGroup
            name={"radio-doc/c"}
        ...
        <P>Disable radios by adding <Pre>{`{ disabled: true }`}</Pre> to options. Disabled radios cannot be manually selected.</P>
    ...
    onChange={({ value }) => setAnimal(value)}/>`
        ...
        <P>If you need to customize the apperance of radios, pass in a function returning Figma nodes or components to the <Pre>render</Pre> prop.</P>
        ...
        <RadioGroup
            ...
            render={({ checked, option: { label, value } }) => (
                <AutoLayout
                    ...
                    cornerRadius={6}
                >
                    <AutoLayout width={24} height={24} padding={4} verticalAlignItems={"center"} horizontalAlignItems={"center"}>
