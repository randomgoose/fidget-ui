...
export function IconsDoc() {
    ...
    return <AutoLayout {...docStyle} name={"Icons Doc"} width={"hug-contents"}>
        <Heading as="h1">Icons</Heading>
        <P>Fidget UI bundles over 500 icons from <Span fill={colors.blue[500]} href="https://heroicons.com/">Heroicons</Span>.</P>
        <Divider />

        <Heading as="h5">Usage</Heading>
        <P>Use icons as components. For example, <Pre>{`<IconAcademicCapSolid />`}.</Pre> You can specify the <Pre>width, height</Pre> and <Pre>color</Pre> props of an Icon.</P>
        <AutoLayout spacing={8}>
            <IconCogSolid />
            <IconCogSolid width={16} height={16} color={colors.cyan[500]} />
        </AutoLayout>
        ...
        <Tabs
            ...
            items={[
                {
                    tab: "Solid", key: "solid", children: (
                        <SimpleGrid columns={12} spacing={8} width={"hug-contents"}>
                            {...solidIconEntries.filter(([k]) => k.toLowerCase().includes(keyword.toLowerCase())).map(([k, Icon]) => <IconDisplay name={k} component={Icon} key={k} />)}
                        </SimpleGrid>
                    )
                },
                {
                    tab: "Outline", key: "outline", children: (
                        <SimpleGrid columns={12} spacing={8} width={"hug-contents"}>
                            {...outlineIconEntries.filter(([k]) => k.toLowerCase().includes(keyword.toLowerCase())).map(([k, Icon]) => <IconDisplay name={k} component={Icon} key={k} />)}
                        </SimpleGrid>
                    )
                }
            ]}
