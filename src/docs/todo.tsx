import { Heading, IconButton, IconMagnifyingGlass, IconMinus, IconPlus, TextField } from "../components";
...
export function Todo() {
    ...
    >
        <AutoLayout width={"fill-parent"} verticalAlignItems={"center"}>
            <Heading as="h5" width={"fill-parent"}>Todo</Heading>
            <IconButton
                icon={<IconPlus />}
                size={"sm"}
                variant={"ghost"}
        ...
        <TextField
            ...
            leftElement={<IconMagnifyingGlass width={"fill-parent"} height={"fill-parent"} color={colors.neutral[400]} />}
        ...
        <AutoLayout name="Todo List" width={"fill-parent"} direction={"vertical"}>
            {
                todos.length > 0 ? todos.map((todo) => (
                    ...
                    : (
                        <AutoLayout width={"fill-parent"} height={72} verticalAlignItems={"center"} horizontalAlignItems={"center"}>
...
function TodoItem(props: TodoProps & { onChange?: (todo: TodoProps) => void; onDelete?: (id: string) => void; }) {
    const { onChange, onDelete, ...todo } = props;

    return (
        <AutoLayout name="Todo Item" verticalAlignItems={"center"} width={"fill-parent"}>
            <Checkbox colorScheme={"emerald"} checked={props.done} onChange={(checked) => onChange && onChange({ ...todo, done: checked })} />
            <TextField
                width={"fill-parent"}
                value={props.content}
                ...
                showClearIcon={false}
            />
            <IconButton variant="ghost" icon={<IconMinus color={colors.neutral[300]} />} size={"sm"} onClick={() => onDelete && onDelete(props.id)} />
        </AutoLayout>
    )
}